...
function toMongoArgs(gqlFilter, lastFieldType) {
  ...
  _.each(gqlFilter, (v, k) => {
    if (_.isPlainObject(v)) {
      ...
      } else if (k === `nin` && lastFieldType.constructor.name === `GraphQLList`) {
...
function _runQuery() {
  _runQuery = (0, _asyncToGenerator2.default)(function* ({
    ...
    if (gqlArgs.sort) {
      const sortFields = toSortFields(gqlArgs.sort); // Create an index for each sort field. Indexing requires sorting
